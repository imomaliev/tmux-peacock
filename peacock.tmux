...
main() {
    ...
    tmux set-hook -g session-created "run-shell '${session_created_hook:-$default_session_created_hook}'"
    tmux set-hook -g session-renamed "run-shell '${session_created_hook:-$default_session_created_hook}'"
    # using additional window hook because some settings are not applied properly
    # on 'session-created' hook
    tmux set-hook -g window-linked "run-shell '${window_created_hook:-$default_window_created_hook}'"
